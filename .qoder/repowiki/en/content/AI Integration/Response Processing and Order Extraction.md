...
## Project Structure
The project follows a modular structure with distinct components for the core logic, user interface, and configuration. The main application is driven by `main.py`, which sets up the Gradio web interface and handles user interactions. The core agent logic, including order processing and voice synthesis, is encapsulated in `bartending_agent.py`. Configuration and dependencies are managed through `requirements.txt`, while various Jupyter notebooks in the `notebooks/` directory suggest iterative development and experimentation phases. The `README.md` provides basic setup instructions for running the application locally.
...
## Core Components
The system's functionality is built around two primary components: the Gradio-based user interface in `main.py` and the stateless agent logic in `bartending_agent.py`. The UI manages the session state, including conversation history and order details, while the agent module contains the business logic for processing user input, interacting with the LLM, and generating voice responses. The separation of concerns allows the agent to be stateless, receiving the current session state as input and returning the updated state, which promotes reusability and simplifies testing. Key functions include `process_order()` for handling user requests and `get_voice_audio()` for generating speech from text.
...
The response processing pipeline begins when a user submits a message through the Gradio interface. The `handle_gradio_input` function in `main.py` is triggered, which calls the `process_order` function from `bartending_agent.py`, passing the current session history and order state. The `process_order` function constructs a detailed prompt for the Gemini LLM, incorporating the menu, current order, conversation history, and the latest user input. This prompt is designed to guide the LLM to respond as a helpful bartender, providing appropriate answers based on the context.
...
**Section sources**
- [bartending_agent.py](file://bartending_agent.py#L121-L132)
- [bartending_agent.py](file://bartending_agent.py#L167-L299)

## Integration with TTS System
The system integrates with the Cartesia TTS service to provide audio feedback to the user. After the `process_order` function returns a text response, the `handle_gradio_input` function in `main.py` calls the `get_voice_audio` function from `bartending_agent.py` to generate a voice recording of the response. The `get_voice_audio` function uses the Cartesia client to synthesize speech, returning the audio data as WAV bytes.
...
**Section sources**
- [bartending_agent.py](file://bartending_agent.py#L267-L299)
- [bartending_agent.py](file://bartending_agent.py#L319-L372)
